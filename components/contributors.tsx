...
function removeDuplicateRoles(roles:any) {
  return roles.filter((item:any, index:any) => roles.indexOf(item) === index);
}

function addPrimary(persons:any, primary:any) {
  persons = Array.isArray(persons) ? persons : [];

  if (primary !== undefined) {
    let existing = persons.filter((item:any, index:any) => item?.name === primary?.name);
...
function RoleLinks(props:any) {
  ...
  if (person !== undefined && index !== undefined) {
    if ('role' in person && Array.isArray(person.role) && person.role.length > 0) {
      ...
      return roles.map((role:any, idx:any) => (
