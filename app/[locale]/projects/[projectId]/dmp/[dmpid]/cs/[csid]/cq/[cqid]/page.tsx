...
const PlanOverviewCustomQuestionPage: React.FC = () => {
  const params = useParams();
  const router = useRouter();
  const dmpId = params.dmpid as string;
  const projectId = params.projectId as string;
  const versionedSectionId = params.csid as string;
  const versionedQuestionId = params.cqid as string;
  ...
  /*GraphQL queries */
  const {
    data: customSelectedQuestion,
    loading: customSelectedQuestionLoading,
    error: customSelectedQuestionError
  } = useQuery(
    PublishedCustomQuestionDocument,
    {
      variables: {
        versionedCustomQuestionId: Number(versionedQuestionId)
      },
  ...
  const {
    ...
    customSectionId: Number(versionedSectionId),
    customQuestionId: Number(versionedQuestionId)
  });
  ...
  // Get parsed JSON from custom question, and set parsed, question and questionType in state
  useEffect(() => {
    if (customSelectedQuestion) {
      const q = customSelectedQuestion.publishedCustomQuestion;
      const cleanedQuestion = {
        ...
        ownerAffiliation: q?.ownerAffiliation ?? null,
      };

      try {
        const { parsed, error } = getParsedQuestionJSON(cleanedQuestion, routePath('projects.dmp.customQuestion.underCustomSection', { projectId, dmpId, csid: versionedSectionId, cqid: versionedQuestionId }), Global);
        if (!parsed?.type) {
          if (error) {
            logECS('error', 'Parsing error', {
              error: 'Invalid question type in parsed JSON',
              url: { path: routePath('projects.dmp.customQuestion.underCustomSection', { projectId, dmpId, csid: versionedSectionId, cqid: versionedQuestionId }) }
            });
            setErrors([error]);
          }
          return;
  ...
  useEffect(() => {
    if (planData?.plan) {
      ...
      const sectionBelongsToPlan = planSections && planSections.some(section => section.customSectionId === Number(versionedSectionId));
  ...
  if (customSelectedQuestionLoading || planQueryLoading || answerLoading) {
    return <Loading message={Global('messaging.loading')} />;
  }

  if (customSelectedQuestionError || planQueryError || answerError) {
...
export default PlanOverviewCustomQuestionPage;