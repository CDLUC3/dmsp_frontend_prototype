...
const PlanOverviewCustomSectionPage: React.FC = () => {
  ...
  const sectionId = Number(params.csid); // custom section id
  ...
  const { data: customQuestionsData, loading: customQuestionsLoading, error: customQuestionsError } = useQuery(PublishedCustomQuestionsDocument, {
  ...
  const section = useMemo(() => {
    if (customSectionData?.publishedCustomSection) {
  ...
  const questions: VersionedQuestion[] = useMemo(() => {
    const source = customQuestionsData?.publishedCustomQuestions;
    ...
    return source
      ...
      .map((q) => ({
        ...
        link: routePath('projects.dmp.customQuestion.underCustomSection', {
          projectId, dmpId, csid: sectionId, cqid: String(q.customQuestionId)
        }),
  ...
  if (customQuestionsLoading || customSectionLoading || planLoading) {
    return <Loading />;
  }


  if (customQuestionsError) {
    return <div>{Section('errors.errorLoadingSections', { message: customQuestionsError.message })}</div>;
  }
