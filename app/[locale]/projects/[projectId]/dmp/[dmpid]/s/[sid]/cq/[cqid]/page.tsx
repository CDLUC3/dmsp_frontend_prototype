...
const PlanOverviewCustomQuestionUnderVersionedSectionPage: React.FC = () => {
  ...
  useEffect(() => {
    if (customSelectedQuestion) {
      ...
      try {
        const { parsed, error } = getParsedQuestionJSON(cleanedQuestion, routePath('projects.dmp.customQuestion.underVersionedSection', { projectId, dmpId, sid: versionedSectionId, cqid: versionedQuestionId }), Global);
        if (!parsed?.type) {
          if (error) {
            logECS('error', 'Parsing error', {
              error: 'Invalid question type in parsed JSON',
              url: { path: routePath('projects.dmp.customQuestion.underVersionedSection', { projectId, dmpId, sid: versionedSectionId, cqid: versionedQuestionId }) }
            });
            setErrors([error]);
          }
          return;
