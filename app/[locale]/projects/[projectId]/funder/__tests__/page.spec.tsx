...
describe('ProjectsProjectFunding', () => {
  ...
  it('should render the "Add a funder" button and handles click', async () => {
    render(<ProjectsProjectFunding />);
    const addButton = screen.getByRole('button', { name: 'Add a funder' });
    expect(addButton).toBeInTheDocument();

    fireEvent.click(addButton);
    await waitFor(() => {
      // Should redirect to the Feeback page when modal is closed
      expect(mockUseRouter().push).toHaveBeenCalledWith('/en-US/projects/proj_2425/funder/search');
    });
  });

  it('should render the funder list and handles "Edit" button click', async () => {
    render(<ProjectsProjectFunding />);
    ...
    await waitFor(() => {
      // Should redirect to the Feeback page when modal is closed
      expect(mockUseRouter().push).toHaveBeenCalledWith('/en-US/projects/proj_2425/funder/edit');
    });
  });
