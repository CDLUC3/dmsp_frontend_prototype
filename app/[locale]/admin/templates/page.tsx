...
const OrganizationTemplateListPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    processTemplates({
      ...
      items: (templateData.myTemplates.items ?? []).filter(
        (item): item is TemplateSearchResultInterface => item !== null,
      ),
    });
            ...
            >
              {searchResults.map((template, index) => (
                <div
                  ...
                  data-index={index}
                >
                  <TemplateSelectListItem item={template} />
                </div>
              ))}
