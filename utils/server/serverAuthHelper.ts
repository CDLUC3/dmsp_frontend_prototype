...
class ServerAuthError extends Error {
  ...
  constructor({
    ...
    logger.error(prepareObjectForLogs({ statusMsg, source }), "ServerAuthError");
  }
}
...
export const serverRefreshAuthTokens = async () => {
  try {
    ...
    logger.error(prepareObjectForLogs({ err }), "Error refreshing auth token");
    return null;
  }
};
...
export const serverFetchCsrfToken = async () => {
  try {
    ...
    logger.error(prepareObjectForLogs({ err }),"Error getting CSRF token");
    return null;
  }
};
